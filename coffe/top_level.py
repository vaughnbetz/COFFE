...
def generate_pgateoutputcrossbar_top(name, maxwidth, def_use_tgate):
    ...
    the_file.write("Xff n_1_2 n_hang2 vdd gnd vdd gnd gnd vdd gnd vdd vdd gnd ff\n")
...
def generate_lut_and_driver_top(input_driver_name, input_driver_type, use_tgate, use_fluts, updates = False):
    ...
    if input_driver_type == "default_rsel" or input_driver_type == "reg_fb_rsel":
        # For the new design A is connected to ff2 while B is connected to ff3 this is controlled by the string (n)  
        spice_file.write("Xff"+n+" n_rsel n_ff_out n_mux_out vsram vsram_n gnd vdd gnd vdd gnd vdd vdd gnd ff"+n+"\n\n")
    ...
    if not updates:
        ...
        if lut_letter in ('d', 'e', 'f'):
            ...
            if lut_letter in ('e', 'f'):
                # flut mux level 3
                spice_file.write(utils.create_wire("n_1_6", "n_1_7", "fmux_l2", "fmux_l3"))
...
def generate_general_ble_output_top(name, use_tgate, use_fluts, updates = False, input_size = 2):
    """ """
    ...
    if updates == 4:
        if input_size == 3:
            targ1 = "Xfmux_l3_load.Xgeneral_ble_output3.Xgeneral_ble_output3_driver.n_1_1"
    ...
    if updates in (1, 2, 3):
        ...
        if input_size == 2:
            top_file.write("Xlut n_in n_0_1 "+lut_input_nodes+"vdd gnd lut\n\n")
            top_file.write("* Fmux level 1 (Fmux level 1 and the FA) \n")
            ...
            top_file.write("Xff2 n_0_4 n_hang2 n_1_1 vdd gnd vsram vsram_n gnd vdd gnd vdd vdd gnd ff2\n\n")
            top_file.write("* Wire connecting the output of the ff input select mux output and the generabl ble output 2:1 mux\n")
            top_file.write(utils.create_wire("n_1_1", "n_1_2", "ff2_mux", "gbo2"))
            top_file.write("Xgeneral_ble_output n_1_2 n_general_out vsram vsram_n vdd_general_output gnd general_ble_output\n\n")
        elif input_size == 3:
            top_file.write("Xlut n_in n_0_1 "+lut_input_nodes+"vdd gnd lut\n\n")
            top_file.write("* Fmux level 1 (Fmux level 1 and the FA)\n")
            top_file.write("Xflut_output_load n_0_1 vdd n_0_2 vdd n_hang ncout nsumout vdd gnd vdd vdd vdd flut_output_load\n\n")
...
def generate_ff_top(name, use_tgate, updates, input_size = 2):
    ...
    if updates in (1, 2, 3):
        ...
        top_file.write("Xgeneral_ble_output n_1_4 n_general_out vsram vsram_n vdd gnd general_ble_output\n\n")
        #else:
...
def generate_flut_mux_top(name, use_tgate, enable_carry_chain, level, updates):
    ...
    if updates:
        ...
        if level == 1:
            ...
            n_1_2 = "n_1_4"
            if updates == 4:
                fname = "fmux_l1_load.Xfmux_l2"
                n_out = "n_1_7"
                n_1_2 = "Xfmux_l1_load.n_1_1"
        elif level == 3:
            vdd_f3 = "vdd_f"
            output = "n_1_7"
            n_1_2 = "n_1_6"
    ...
    # TODO: combine the updates with the old desing more neatly
    if not updates:
        ...
        if enable_carry_chain == 1:
            ...
            # BUG: This is a bug fix for COFFE 2.0 uncomment this and push it to the master
        ...
        if updates in (1, 2, 3):
            ...
            # fmux_l2 and fmux_l2_duplicate loading
            top_file.write("* Loading at the output of fmux_l1 containing fmux_l2 and fmux_l2_duplicate and their input wires\n")
            top_file.write("Xfmux_l1_load n_1_3 vdd n_1_5 vdd n_dup_out vdd gnd "+vdd_f2+" vdd fmux_l1_load\n\n")
        ...
        if updates == 4:
            if level > 1:
                top_file.write(utils.create_wire("n_1_5", "n_1_6", "fmux_l2", "fmux_l3"))
                top_file.write("Xfmux_l3 n_1_6 n_1_7 vdd gnd "+vdd_f3+" gnd fmux_l3\n\n")
...
def generate_cc_in_out_top(name, use_tgate, updates, id):
    ...
    if updates in (1, 4):
        ...
        top_file.write("Xcarrychain2 n_1_4 gnd vdd n_cout2 nn_sout2 n_p_4 vdd gnd FA_carry_chain2\n")
        ...
        if id == 2:
            ...
            top_file.write("Xff"+fn+"_2 n_2_1 n_2_2 n_mux_out vsram vsram_n vdd gnd gnd vdd gnd vdd vdd gnd ff"+fn+"\n\n")
...
def generate_carry_chain_ripple_top(name, updates, id):
    ...
    if updates in (1, 4):
        ...
        if id == 1:
            ...
            top_file.write("\n* Reg"+fn+" in Stratix 10 architecture with a "+fn+":1 mux at its input\n")
            top_file.write("Xff"+fn+" n_1_4 n_1_5 n_mux_out vsram vsram_n vdd gnd gnd vdd gnd vdd vdd gnd ff"+fn+"\n\n")        
    else:
...
def generate_dedicated_driver_top(name, top_name, num_bufs):
    ...
    top_file.write("Xff n_1_3 n_hang2 n_mux_out vdd gnd vdd gnd gnd vdd gnd vdd vdd gnd ff\n")
