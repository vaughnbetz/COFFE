...
class _Specs:
    ...
    def __init__(self, arch_params_dict, quick_mode_threshold):
        ...
        self.lut_design             = arch_params_dict['lut_design']
...
class _LocalMUX(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        if not self.use_tgate :
            ...
            if (GB_LUT > 0 and LEVEL_SHIFTER > 0) or voltage_islands > 0 : 
                self.transistor_names, self.wire_names = mux_subcircuits.generate_ptran_2lvl_mux_no_driver_gb_lut_level_shifter(subcircuit_filename, self.name, self.implemented_size, self.level1_size, self.level2_size)
            else :
            ...
            self.initial_transistor_sizes["rest_" + self.name + "_pmos"] = 1
            self.initial_transistor_sizes["inv_" + self.name + "_1_nmos"] = 4
            self.initial_transistor_sizes["inv_" + self.name + "_1_pmos"] = 4     
            # lvl shifter transistors
            #self.initial_transistor_sizes["tran_lvl_shifter_" + self.name + "_1_pd"] = 1
            #self.initial_transistor_sizes["tran_lvl_shifter_" + self.name + "_1_pu"] = 1
            #self.initial_transistor_sizes["tran_lvl_shifter_" + self.name + "_2_pd"] = 1
            #self.initial_transistor_sizes["tran_lvl_shifter_" + self.name + "_2_pu"] = 1 
            

            if (GB_LUT > 0 and LEVEL_SHIFTER > 0) or voltage_islands > 0 :             
                self.initial_transistor_sizes["inv_lvl_shifter_" + self.name + "_1_nmos"] = 1
                self.initial_transistor_sizes["inv_lvl_shifter_" + self.name + "_1_pmos"] = 1
                self.initial_transistor_sizes["inv_lvl_shifter_" + self.name + "_2_nmos"] = 1
                self.initial_transistor_sizes["inv_lvl_shifter_" + self.name + "_2_pmos"] = 1             
        else :
            # Call MUX generation function
    ...
    def update_area(self, area_dict, width_dict):
        ...
        if not self.use_tgate :
            ...
            if (GB_LUT > 0 and LEVEL_SHIFTER > 0) or voltage_islands > 0 :                    
                    area = (area + area_dict["inv_lvl_shifter_" + self.name + "_1"] +   
                    area_dict["inv_lvl_shifter_" + self.name + "_2"]) 
                    #area_dict["tran_lvl_shifter_" + self.name + "_1_pd"] +
                    #area_dict["tran_lvl_shifter_" + self.name + "_1_pu"] +
                    #area_dict["tran_lvl_shifter_" + self.name + "_2_pd"] +
                    #area_dict["tran_lvl_shifter_" + self.name + "_2_pu"])                    
        else :
            area = ((self.level1_size*self.level2_size)*area_dict["tgate_" + self.name + "_L1"] +
                    self.level2_size*area_dict["tgate_" + self.name + "_L2"] +
                    # area_dict["rest_" + self.name + ""] +
...
class _LUTInputDriver(_SizableCircuit):
    ...
    def __init__(self, name, type, delay_weight, tgate_blocks, use_fluts, lut_design):
    ...
    def generate_top(self):
        ...
        is_inverted = False
        if self.lut_design == 1:
            is_inverted = True

        top_level.generate_lut_and_driver_top(self.name, self.type, self.use_tgate, self.use_fluts, is_inverted)       
...
class _LUTInput(_CompoundCircuit):
    ...
    def __init__(self, name, Rsel, Rfb, delay_weight, tgate_blocks, use_fluts, lut_design):
        ...
        self.driver = _LUTInputDriver(name, self.type, delay_weight, tgate_blocks, use_fluts, lut_design)
...
class _LUTInputDriverLoad:
    ...
    def __init__(self, name, tgate_blocks, use_fluts, predecode, predecode_input):
        ...
        self.predecode_input = predecode_input
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers, ratio):
        ...
        if self.predecode == "on": 
            wire_lengths["wire_driver_predecode_L1"] =  2*width_dict["predecoder"]
            if self.predecode_input == "c":
                wire_lengths["wire_driver_predecode_L1"] = wire_lengths["wire_driver_predecode_L1"] + 2*width_dict["predecoder"]
        # Update set wire layers
        wire_layers["wire_lut_" + self.name + "_driver_load"] = 0
        if self.predecode == "on": 
            wire_layers["wire_driver_predecode_L1"] = 0
    ...
    def generate(self, subcircuit_filename, K):
        ...
        if not self.use_tgate :
            ...
            if self.name == "a":
                if self.predecode == "on": # if lut is predecoded then a must be predecoded
                    self.wire_names = lut_subcircuits.generate_lut_driver_load_for_decoded_inputs(subcircuit_filename, self.name, K, self.use_fluts, self.predecode_input)
                else:
                    self.wire_names = lut_subcircuits.generate_ptran_lut_driver_load(subcircuit_filename, self.name, K, self.use_fluts, )
            elif self.name == "b": # if lut is predecoded then b must be predecoded
                if self.predecode == "on":             
                    self.wire_names = lut_subcircuits.generate_lut_driver_load_for_decoded_inputs(subcircuit_filename, self.name, K, self.use_fluts, self.predecode_input)
                else:
                    self.wire_names = lut_subcircuits.generate_ptran_lut_driver_load(subcircuit_filename, self.name, K, self.use_fluts)    
            elif self.name == "c": 
                if ((ord(self.name) <= ord(self.predecode_input))): # check if c is predecoded
                    print "loading c driver with predecoder ibrahim"
                    self.wire_names = lut_subcircuits.generate_lut_driver_load_for_decoded_inputs(subcircuit_filename, self.name, K, self.use_fluts, self.predecode_input)
                else:
                    self.wire_names = lut_subcircuits.generate_ptran_lut_driver_load(subcircuit_filename, self.name, K, self.use_fluts)
            elif self.name == "d":
            ...
            if self.name == "a":
                if self.predecode == "on": # if lut is predecoded then a must be predecoded
                    self.wire_names = lut_subcircuits.generate_lut_driver_load_for_decoded_inputs_tg_lut(subcircuit_filename, self.name, K, self.use_fluts, self.predecode_input)
                else:			
                    self.wire_names = lut_subcircuits.generate_tgate_lut_driver_load(subcircuit_filename, self.name, K, self.use_fluts)
            elif self.name == "b":
                if self.predecode == "on": # if lut is predecoded then b must be predecoded
                    self.wire_names = lut_subcircuits.generate_lut_driver_load_for_decoded_inputs_tg_lut(subcircuit_filename, self.name, K, self.use_fluts, self.predecode_input)
                else:			
                    self.wire_names = lut_subcircuits.generate_tgate_lut_driver_load(subcircuit_filename, self.name, K, self.use_fluts)
            elif self.name == "c": 
...
class _LUTInputPredecodeLoad:
    """ LUT input driver load. This load consists of a wire as well as the gates
        of a particular level in the LUT. """

    def __init__(self, name, tgate_blocks, use_fluts, predecode_input):
        self.name = name
        self.use_tgate = "lut" in tgate_blocks
        self.use_fluts = use_fluts
        self.predecode_input = predecode_input
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers, ratio):
        ...
        # Update wire lengths
        wire_lengths["wire_lut_" + self.name + "_predecode_load"] = width_dict["lut"] * ratio
        wire_lengths["wire_lut_" + self.name + "_driver_load"] = width_dict["lut"] * ratio

        # Update set wire layers
        wire_layers["wire_lut_" + self.name + "_predecode_load"] = 0
        
        
    def generate(self, subcircuit_filename, K):
        
        print "Generating LUT " + self.name + "-predcode driver load"
        
        if not self.use_tgate :
            # Call generation function based on input
            if self.name == "a":
                self.wire_names = lut_subcircuits.generate_ptran_lut_predecode_load(subcircuit_filename, self.name, K, self.use_fluts, self.predecode_input)
            elif self.name == "b":
                self.wire_names = lut_subcircuits.generate_ptran_lut_predecode_load(subcircuit_filename, self.name, K, self.use_fluts, self.predecode_input)
            elif self.name == "c":
                self.wire_names = lut_subcircuits.generate_ptran_lut_predecode_load(subcircuit_filename, self.name, K, self.use_fluts, self.predecode_input)
            ...
            # Call generation function based on input
            if self.name == "a":
                self.wire_names = lut_subcircuits.generate_tgate_lut_predecode_load(subcircuit_filename, self.name, K, self.use_fluts, self.predecode_input)
            elif self.name == "b":
                self.wire_names = lut_subcircuits.generate_tgate_lut_predecode_load(subcircuit_filename, self.name, K, self.use_fluts, self.predecode_input)
            elif self.name == "c":
                self.wire_names = lut_subcircuits.generate_tgate_lut_predecode_load(subcircuit_filename, self.name, K, self.use_fluts, self.predecode_input)
    ...
    def print_details(self):
        print "LUT input predecode load details."
...
class _LUT(_SizableCircuit):
    ...
    def __init__(self, K, Rsel, Rfb, tgate_blocks, use_finfet, use_fluts, lut_design):
        # Name of LUT 
        self.name = "lut"
        self.use_fluts = use_fluts
        # Size of LUT
        ...
        self.input_predecode_loads = {}
        self.predecode = "on" # on or off
        self.predecode_input = "b" # a or c
        self.decoder_design = 3 # 1 or 2 or 3		
        # Delay weight in a representative critical path
        ...
        for i in range(tempK):
            ...
            self.input_drivers[name] = _LUTInput(name, Rsel, Rfb, delay_weight, tgate_blocks, use_fluts, lut_design)
            self.input_driver_loads[name] = _LUTInputDriverLoad(name, tgate_blocks, use_fluts, self.predecode, self.predecode_input )

    
        if self.predecode == "on":    
            self.input_predecode_loads["a"] = _LUTInputPredecodeLoad("a", tgate_blocks, use_fluts, self.predecode_input)
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        if tempK == 6:
            if self.predecode == "on": 
                init_tran_sizes = self._generate_6lut_predecode(subcircuit_filename, min_tran_width, self.use_tgate, self.use_finfet, self.use_fluts, self.predecode_input, self.decoder_design)
            elif self.lut_design == 0:
                init_tran_sizes = self._generate_6lut_lut_design0(subcircuit_filename, min_tran_width, self.use_tgate, self.use_finfet, self.use_fluts)           
            elif self.lut_design == 1:
                init_tran_sizes = self._generate_6lut_lut_design1(subcircuit_filename, min_tran_width, self.use_tgate, self.use_finfet, self.use_fluts)         
            elif self.lut_design == 2:
                init_tran_sizes = self._generate_6lut_lut_design2(subcircuit_filename, min_tran_width, self.use_tgate, self.use_finfet, self.use_fluts)        
            else:
    ...
    def generate_top(self):
        ...
        if tempK == 6:
            if self.lut_design == 0:
                self.top_spice_path = top_level.generate_lut6_top_design0(self.name, self.use_tgate)
            elif self.lut_design == 1:
                self.top_spice_path = top_level.generate_lut6_top_design1(self.name, self.use_tgate)
            elif self.lut_design == 2:
                self.top_spice_path = top_level.generate_lut6_top_design2(self.name, self.use_tgate)
            else:
                self.top_spice_path = top_level.generate_lut6_top(self.name, self.use_tgate, self.predecode, self.decoder_design)
        elif tempK == 5:
    ...
    def update_area(self, area_dict, width_dict):
        ...
        if not self.use_tgate :
            ...
            if tempK == 6:    
                area += (64*area_dict["inv_lut_0sram_driver_2"] + 
                        ...
                        #32*area_dict["ptran_lut_L2"] + predecode_ib
                ...
                if self.lut_design < 0:
                    area += 8*area_dict["rest_lut_int_buffer"]
                    area += 8*area_dict["inv_lut_int_buffer_1"]
                    area += 8*area_dict["inv_lut_int_buffer_2"]
                    area += area_dict["inv_lut_out_buffer_2"]
                ...
                if self.lut_design == 1:
                    area += 16*area_dict["rest_lut_int_buffer1"]
                    area += 16*area_dict["inv_lut_int_buffer_1"]
                    area += 4*area_dict["rest_lut_int_buffer2"]
                    area += 4*area_dict["inv_lut_int_buffer_2"]

                if self.lut_design == 2:
                    area += 16*area_dict["rest_lut_int_buffer"]
                    area += 16*area_dict["inv_lut_int_buffer_1"]
                    area += 16*area_dict["inv_lut_int_buffer_2"]

                    area += 4*area_dict["rest_lut_int_buffer_2"]
                    area += 4*area_dict["inv_lut_int_buffer_2_1"]
                    area += 4*area_dict["inv_lut_int_buffer_2_2"]

                    area += area_dict["inv_lut_out_buffer_2"]

                if self.predecode == "on":
                    if (self.predecode_input != "c"): # decoding the first two inputs only, so lets add 4 decoders and layer 3 of the mux tree
                        predecoder_area += (8*area_dict["inv_nand_lut_predecode_1"]
						+ 4*area_dict["inv_lut_predecode_2"])
                        area += 16*area_dict["ptran_lut_L3"]
						
						# ibrahim remove 4 of the internal buffers trial as we are buffering after d
                        area -= 4*area_dict["inv_lut_int_buffer_1"]
                        area -= 4*area_dict["inv_lut_int_buffer_2"]
						
                    else: # add 8 3-input nand gates + 8 inverters
                        predecoder_area += (24*area_dict["inv_nand_lut_predecode_1"] + 
							8*area_dict["inv_lut_predecode_2"])
							
                    area += predecoder_area
                else:
                    area += 32*area_dict["ptran_lut_L2"]
                    area += 16*area_dict["ptran_lut_L3"]
            ...
            if tempK == 6:    
                ...
                if self.predecode == "on":
                    if (self.predecode_input != "c"): # decoding the first two inputs only, so lets add 4 decoders and layer 3 of the mux tree
                        ...
                        if (self.decoder_design == 1):						
                            predecoder_area+= 4*area_dict["inv_lut_predecode_2_1"] + 4*area_dict["inv_lut_predecode_2_2"]
                        elif (self.decoder_design == 3):						
                            predecoder_area+= 4*area_dict["inv_lut_predecode_3"]
							
                        area += 16*area_dict["tgate_lut_L3"]
                        ...
                        if (self.decoder_design == 1):						
                            predecoder_area+= 8*area_dict["inv_lut_predecode_2_1"] + 8*area_dict["inv_lut_predecode_2_2"]
                        elif (self.decoder_design == 3):						
                            predecoder_area+= 8*area_dict["inv_lut_predecode_3"]							
                    ...
                    area += 32*area_dict["tgate_lut_L2"]
                    area += 16*area_dict["tgate_lut_L3"]					
        ...
        width_dict["predecoder"] = math.sqrt(predecoder_area)
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers, lut_ratio):
        ...
        if not self.use_tgate :
            if self.K == 6:        
                ...
                if self.lut_design < 0:
                    ...
                    wire_lengths["wire_lut_int_buffer_out"] = (width_dict["inv_lut_int_buffer_2"] + width_dict["ptran_lut_L5"])/4 #make it 4 depending on the buf stage
                    wire_lengths["wire_lut_out_buffer"] = (width_dict["inv_lut_out_buffer_1"] + width_dict["inv_lut_out_buffer_2"])/4

                if self.lut_design == 0:
                    wire_lengths["wire_lut_int_buffer"] = (width_dict["inv_lut_int_buffer_1"] + width_dict["ptran_lut_L5"])/4

                if self.lut_design == 1:
                    wire_lengths["wire_lut_int_buffer"] = (width_dict["inv_lut_int_buffer_1"] + width_dict["ptran_lut_L3"])/4
                    wire_lengths["wire_lut_int_buffer_2"] = (width_dict["inv_lut_int_buffer_2"] + width_dict["ptran_lut_L5"])/4

                if self.lut_design == 2:
                    wire_lengths["wire_lut_int_buffer"] = (width_dict["inv_lut_int_buffer_1"] + width_dict["inv_lut_int_buffer_2"])/4
                    wire_lengths["wire_lut_int_buffer_out"] = (width_dict["inv_lut_int_buffer_2"] + width_dict["ptran_lut_L3"])/4 #make it 4 depending on the buf stage
                    wire_lengths["wire_lut_out_buffer"] = (width_dict["inv_lut_out_buffer_1"] + width_dict["inv_lut_out_buffer_2"])/4

                    wire_lengths["wire_lut_int_buffer_2"] = (width_dict["inv_lut_int_buffer_2_1"] + width_dict["inv_lut_int_buffer_2_2"])/4
                    wire_lengths["wire_lut_int_buffer_2_out"] = (width_dict["inv_lut_int_buffer_2_2"] + width_dict["ptran_lut_L5"])/4 #make it 4 depending on the buf stage
                    wire_lengths["wire_lut_out_buffer"] = (width_dict["inv_lut_out_buffer_1"] + width_dict["inv_lut_out_buffer_2"])/4

                if self.predecode == "on":
                    wire_lengths["wire_lut_nand_inv_1"] = (width_dict["predecoder"])/4
                ...
                wire_layers["wire_lut_int_buffer_2"] = 0
                wire_layers["wire_lut_int_buffer_2_out"] = 0
                ...
                if self.predecode == "on":
                    wire_layers["wire_lut_nand_inv_1"] = 0
            ...
            if self.K == 6:        
                ...
                if self.lut_design < 0:
                    wire_lengths["wire_lut_int_buffer"] = (width_dict["inv_lut_int_buffer_1"] + width_dict["inv_lut_int_buffer_2"])/4
                    wire_lengths["wire_lut_int_buffer_out"] = (width_dict["inv_lut_int_buffer_2"] + width_dict["tgate_lut_L5"])/4 #make it 4 depending on the buf stage
                    wire_lengths["wire_lut_out_buffer"] = (width_dict["inv_lut_out_buffer_1"] + width_dict["inv_lut_out_buffer_2"])/4

                if self.lut_design == 0:
                    wire_lengths["wire_lut_int_buffer"] = (width_dict["inv_lut_int_buffer_1"] + width_dict["tgate_lut_L5"])/4

                if self.lut_design == 1:
                    wire_lengths["wire_lut_int_buffer"] = (width_dict["inv_lut_int_buffer_1"] + width_dict["tgate_lut_L3"])/4
                    wire_lengths["wire_lut_int_buffer_2"] = (width_dict["inv_lut_int_buffer_2"] + width_dict["tgate_lut_L5"])/4

                if self.lut_design == 2:
                    wire_lengths["wire_lut_int_buffer"] = (width_dict["inv_lut_int_buffer_1"] + width_dict["inv_lut_int_buffer_2"])/4
                    wire_lengths["wire_lut_int_buffer_out"] = (width_dict["inv_lut_int_buffer_2"] + width_dict["tgate_lut_L3"])/4 #make it 4 depending on the buf stage
                    wire_lengths["wire_lut_out_buffer"] = (width_dict["inv_lut_out_buffer_1"] + width_dict["inv_lut_out_buffer_2"])/4

                    wire_lengths["wire_lut_int_buffer_2"] = (width_dict["inv_lut_int_buffer_2_1"] + width_dict["inv_lut_int_buffer_2_2"])/4
                    wire_lengths["wire_lut_int_buffer_2_out"] = (width_dict["inv_lut_int_buffer_2_2"] + width_dict["tgate_lut_L5"])/4 #make it 4 depending on the buf stage
                    wire_lengths["wire_lut_out_buffer"] = (width_dict["inv_lut_out_buffer_1"] + width_dict["inv_lut_out_buffer_2"])/4
										
                if self.predecode == "on":				
                    wire_lengths["wire_lut_nand_inv_1"] = (width_dict["inv_nand_lut_predecode_1"] + width_dict["inv_lut_predecode_2"])/4
                    if(self.decoder_design == 1):					
                        wire_lengths["wire_lut_nand_inv_2_1"] = (width_dict["inv_nand_lut_predecode_1"] + width_dict["inv_lut_predecode_2_1"])/4
                        wire_lengths["wire_lut_nand_inv_2_2"] = (width_dict["inv_lut_predecode_2_1"] + width_dict["inv_lut_predecode_2_2"])/4
                    elif(self.decoder_design == 3):						
                        wire_lengths["wire_lut_nand_inv_2"] = (width_dict["inv_lut_predecode_2"] + width_dict["inv_lut_predecode_3"])/4
                ...
                if self.predecode == "on":
                    wire_layers["wire_lut_nand_inv_1"] = 0
                    if(self.decoder_design == 1):					
                        wire_layers["wire_lut_nand_inv_2_1"] = 0     
                        wire_layers["wire_lut_nand_inv_2_2"] = 0
                    elif(self.decoder_design == 3):	
                        wire_layers["wire_lut_nand_inv_2"] = 0
        ...
        if self.predecode == "on":    
            # for driver_load_name, input_precode_load in self.input_predecode_loads.iteritems():
            self.input_predecode_loads["a"].update_wires(width_dict, wire_lengths, wire_layers, lut_ratio)    
    ...
    def _generate_6lut_predecode(self, subcircuit_filename, min_tran_width, use_tgate, use_finfet, use_fluts, predecode_input, decoder_design):
        ...
        if not use_tgate :		
            if (predecode_input == "c"):
                self.transistor_names, self.wire_names = lut_subcircuits.generate_ptran_lut6_ib_3_decode(subcircuit_filename, min_tran_width, use_finfet)
                #self.transistor_names, self.wire_names = lut_subcircuits.generate_ptran_lut6_ib_3_decode_buff_2(subcircuit_filename, min_tran_width, use_finfet)
                #self.transistor_names, self.wire_names = lut_subcircuits.generate_ptran_lut6_ib_3_decode_no_internal_buff(subcircuit_filename, min_tran_width, use_finfet)				
            else :
                #self.transistor_names, self.wire_names = lut_subcircuits.generate_ptran_lut6_ib(subcircuit_filename, min_tran_width, use_finfet)
                self.transistor_names, self.wire_names = lut_subcircuits.generate_ptran_lut6_buff_after_3_ib(subcircuit_filename, min_tran_width, use_finfet)
            ...
            self.initial_transistor_sizes["inv_nand_lut_predecode_1_nmos"] = 1
            self.initial_transistor_sizes["inv_nand_lut_predecode_1_pmos"] = 1
            self.initial_transistor_sizes["inv_lut_predecode_2_nmos"] = 4 
            self.initial_transistor_sizes["inv_lut_predecode_2_pmos"] = 4 
            
            
            self.initial_transistor_sizes["ptran_lut_L1_nmos"] = 2
            #self.initial_transistor_sizes["ptran_lut_L2_nmos"] = 2
			
            if (predecode_input != "c"): # only add this layer if two inputs are decoded
                self.initial_transistor_sizes["ptran_lut_L3_nmos"] = 2
			
            self.initial_transistor_sizes["rest_lut_int_buffer_pmos"] = 1 #ib_rem
            self.initial_transistor_sizes["inv_lut_int_buffer_1_nmos"] = 2 #ib_rem
            self.initial_transistor_sizes["inv_lut_int_buffer_1_pmos"] = 2 #ib_rem
            self.initial_transistor_sizes["inv_lut_int_buffer_2_nmos"] = 4 #ib_rem
            self.initial_transistor_sizes["inv_lut_int_buffer_2_pmos"] = 6 #ib_rem
            ...
            if (predecode_input == "c"):
                self.transistor_names, self.wire_names = lut_subcircuits.generate_tgate_lut6_ib_3_decode_buff_2(subcircuit_filename, min_tran_width, use_finfet, decoder_design)		
            # use transmission gates
            else:
                self.transistor_names, self.wire_names = lut_subcircuits.generate_tgate_lut6_ib_2_decode_buff_2(subcircuit_filename, min_tran_width, use_finfet, decoder_design)

            # Give initial transistor sizes
            self.initial_transistor_sizes["inv_lut_0sram_driver_2_nmos"] = 4
            self.initial_transistor_sizes["inv_lut_0sram_driver_2_pmos"] = 6
			# NAND and inverter after it
            self.initial_transistor_sizes["inv_nand_lut_predecode_1_nmos"] = 1
            self.initial_transistor_sizes["inv_nand_lut_predecode_1_pmos"] = 1
            self.initial_transistor_sizes["inv_lut_predecode_2_nmos"] = 4 
            self.initial_transistor_sizes["inv_lut_predecode_2_pmos"] = 4
            ...
            if (decoder_design == 1):
			# 2 inverters to get the same value of the NAND output
                self.initial_transistor_sizes["inv_lut_predecode_2_1_nmos"] = 4 
                self.initial_transistor_sizes["inv_lut_predecode_2_1_pmos"] = 4 

                self.initial_transistor_sizes["inv_lut_predecode_2_2_nmos"] = 4 
                self.initial_transistor_sizes["inv_lut_predecode_2_2_pmos"] = 4  			
            elif (decoder_design == 3):			
                self.initial_transistor_sizes["inv_lut_predecode_3_nmos"] = 4 
                self.initial_transistor_sizes["inv_lut_predecode_3_pmos"] = 4 
        ...
        self.input_predecode_loads["a"].generate(subcircuit_filename, self.K)
    ...
    def _generate_6lut_lut_design0(self, subcircuit_filename, min_tran_width, use_tgate, use_finfet, use_fluts):
        """ This function created the lut subcircuit and all the drivers and driver not subcircuits """
        print "Generating 6-LUT"
        ...
        if not use_tgate :
            # use pass transistors
            self.transistor_names, self.wire_names = lut_subcircuits.generate_ptran_lut6_design0(subcircuit_filename, min_tran_width, use_finfet)
            ...
            self.initial_transistor_sizes["ptran_lut_L3_nmos"] = 2
            self.initial_transistor_sizes["rest_lut_int_buffer_pmos"] = 1
            self.initial_transistor_sizes["inv_lut_int_buffer_1_nmos"] = 6
            self.initial_transistor_sizes["inv_lut_int_buffer_1_pmos"] = 8
            self.initial_transistor_sizes["ptran_lut_L4_nmos"] = 3
            self.initial_transistor_sizes["ptran_lut_L5_nmos"] = 3
            self.initial_transistor_sizes["ptran_lut_L6_nmos"] = 3
            self.initial_transistor_sizes["rest_lut_out_buffer_pmos"] = 1
            self.initial_transistor_sizes["inv_lut_out_buffer_1_nmos"] = 6
            self.initial_transistor_sizes["inv_lut_out_buffer_1_pmos"] = 6
        else :
            # use transmission gates
            self.transistor_names, self.wire_names = lut_subcircuits.generate_tgate_lut6_design0(subcircuit_filename, min_tran_width, use_finfet)
    ...
    def _generate_6lut_lut_design1(self, subcircuit_filename, min_tran_width, use_tgate, use_finfet, use_fluts):
        """ This function created the lut subcircuit and all the drivers and driver not subcircuits """
        print "Generating 6-LUT"
        ...
        if not use_tgate :
            # use pass transistors
            self.transistor_names, self.wire_names = lut_subcircuits.generate_ptran_lut6_design1(subcircuit_filename, min_tran_width, use_finfet)
            ...
            self.initial_transistor_sizes["ptran_lut_L1_nmos"] = 2
            self.initial_transistor_sizes["ptran_lut_L2_nmos"] = 2
            self.initial_transistor_sizes["rest_lut_int_buffer1_pmos"] = 1
            self.initial_transistor_sizes["inv_lut_int_buffer_1_nmos"] = 2
            self.initial_transistor_sizes["inv_lut_int_buffer_1_pmos"] = 2
            self.initial_transistor_sizes["ptran_lut_L3_nmos"] = 2
            self.initial_transistor_sizes["ptran_lut_L4_nmos"] = 3

            self.initial_transistor_sizes["rest_lut_int_buffer2_pmos"] = 1
            self.initial_transistor_sizes["inv_lut_int_buffer_2_nmos"] = 4
            self.initial_transistor_sizes["inv_lut_int_buffer_2_pmos"] = 6
            ...
            self.initial_transistor_sizes["ptran_lut_L6_nmos"] = 3
            self.initial_transistor_sizes["rest_lut_out_buffer_pmos"] = 1
            self.initial_transistor_sizes["inv_lut_out_buffer_1_nmos"] = 4
            self.initial_transistor_sizes["inv_lut_out_buffer_1_pmos"] = 6

        else :
            # use transmission gates
            self.transistor_names, self.wire_names = lut_subcircuits.generate_tgate_lut6_design1(subcircuit_filename, min_tran_width, use_finfet)
            ...
            self.initial_transistor_sizes["tgate_lut_L4_nmos"] = 3
            self.initial_transistor_sizes["tgate_lut_L4_pmos"] = 3
            self.initial_transistor_sizes["inv_lut_int_buffer_2_nmos"] = 2
            self.initial_transistor_sizes["inv_lut_int_buffer_2_pmos"] = 2
            self.initial_transistor_sizes["tgate_lut_L5_nmos"] = 3
            self.initial_transistor_sizes["tgate_lut_L5_pmos"] = 3
            self.initial_transistor_sizes["tgate_lut_L6_nmos"] = 3
            self.initial_transistor_sizes["tgate_lut_L6_pmos"] = 3
    ...
    def _generate_6lut_lut_design2(self, subcircuit_filename, min_tran_width, use_tgate, use_finfet, use_fluts):
        """ This function created the lut subcircuit and all the drivers and driver not subcircuits """
        print "Generating 6-LUT"
        ...
        if not use_tgate :
            # use pass transistors
            self.transistor_names, self.wire_names = lut_subcircuits.generate_ptran_lut6_design2(subcircuit_filename, min_tran_width, use_finfet)
            ...
            self.initial_transistor_sizes["ptran_lut_L3_nmos"] = 2
            self.initial_transistor_sizes["ptran_lut_L4_nmos"] = 3
            self.initial_transistor_sizes["rest_lut_int_buffer_2_pmos"] = 1
            self.initial_transistor_sizes["inv_lut_int_buffer_2_1_nmos"] = 2
            self.initial_transistor_sizes["inv_lut_int_buffer_2_1_pmos"] = 2
            self.initial_transistor_sizes["inv_lut_int_buffer_2_2_nmos"] = 4
            self.initial_transistor_sizes["inv_lut_int_buffer_2_2_pmos"] = 6
            self.initial_transistor_sizes["ptran_lut_L5_nmos"] = 3
            self.initial_transistor_sizes["ptran_lut_L6_nmos"] = 3
            self.initial_transistor_sizes["rest_lut_out_buffer_pmos"] = 1
            self.initial_transistor_sizes["inv_lut_out_buffer_1_nmos"] = 2
            ...
            # use transmission gates
            self.transistor_names, self.wire_names = lut_subcircuits.generate_tgate_lut6_design2(subcircuit_filename, min_tran_width, use_finfet)
            ...
            self.initial_transistor_sizes["inv_lut_int_buffer_2_1_nmos"] = 2
            self.initial_transistor_sizes["inv_lut_int_buffer_2_1_pmos"] = 2
            self.initial_transistor_sizes["inv_lut_int_buffer_2_2_nmos"] = 2
            self.initial_transistor_sizes["inv_lut_int_buffer_2_2_pmos"] = 2
            self.initial_transistor_sizes["tgate_lut_L5_nmos"] = 3
            self.initial_transistor_sizes["tgate_lut_L5_pmos"] = 3
            self.initial_transistor_sizes["tgate_lut_L6_nmos"] = 3
            self.initial_transistor_sizes["tgate_lut_L6_pmos"] = 3
            self.initial_transistor_sizes["inv_lut_out_buffer_1_nmos"] = 2
            self.initial_transistor_sizes["inv_lut_out_buffer_1_pmos"] = 2
            self.initial_transistor_sizes["inv_lut_out_buffer_2_nmos"] = 2
            self.initial_transistor_sizes["inv_lut_out_buffer_2_pmos"] = 2
...
class _BLE(_CompoundCircuit):
    ...
    def __init__(self, K, Or, Ofb, Rsel, Rfb, tgate_blocks, use_finfet, use_fluts, enable_carry_chain, FAs_per_flut, carry_skip_periphery_count, N, lut_design):
        # BLE name
        self.name = "ble"
        ...
        # Create LUT object
        self.lut = _LUT(K, Rsel, Rfb, tgate_blocks, use_finfet, use_fluts, lut_design)
        # Create FF object
...
class _LogicCluster(_CompoundCircuit):
    ...
    def __init__(self, N, K, Or, Ofb, Rsel, Rfb, local_mux_size_required, num_local_mux_per_tile, tgate_blocks, use_finfet, use_fluts, enable_carry_chain, FAs_per_flut, carry_skip_periphery_count, lut_design):
        # Name of logic cluster
        self.name = "logic_cluster"
        # Cluster size
        self.N = N
        # Create BLE object
        self.ble = _BLE(K, Or, Ofb, Rsel, Rfb, tgate_blocks, use_finfet, use_fluts, enable_carry_chain, FAs_per_flut, carry_skip_periphery_count, N, lut_design)
...
class FPGA:
    ...
    def __init__(self, arch_params_dict, run_options, spice_interface):
        ...
        self.logic_cluster = _LogicCluster(self.specs.N, self.specs.K, self.specs.num_ble_general_outputs, self.specs.num_ble_local_outputs, self.specs.Rsel, self.specs.Rfb, 
                                           ...
                                           self.specs.enable_carry_chain, self.specs.FAs_per_flut, self.carry_skip_periphery_count, self.specs.lut_design)
    ...
    def update_delays(self, spice_interface):
        ...
        self.sb_mux.leak_power_state_on = float(spice_meas["meas_on_leakage_power"][0])
        self.sb_mux.leak_power_state_off = float(spice_meas["meas_off_leakage_power"][0])  
        
        print " after switch block\n"
        sys.stdout.flush() 
        ...
        self.cb_mux.leak_power_state_on = float(spice_meas["meas_on_leakage_power"][0])
        self.cb_mux.leak_power_state_off = float(spice_meas["meas_off_leakage_power"][0])      

        print " after connection block\n"
        sys.stdout.flush()         
        ...
        self.logic_cluster.local_mux.leak_power_state_on = float(spice_meas["meas_on_leakage_power"][0])
        self.logic_cluster.local_mux.leak_power_state_off = float(spice_meas["meas_off_leakage_power"][0])   
        
        
        print " after local_mux block\n"
        sys.stdout.flush() 
        ...
        self.logic_cluster.ble.local_output.leak_power_state_on = float(spice_meas["meas_on_leakage_power"][0])
        self.logic_cluster.ble.local_output.leak_power_state_off = float(spice_meas["meas_off_leakage_power"][0])         


        print " after local ble block\n"
        sys.stdout.flush()         
        ...
        self.logic_cluster.ble.general_output.leak_power_state_on = float(spice_meas["meas_on_leakage_power"][0])
        self.logic_cluster.ble.general_output.leak_power_state_off = float(spice_meas["meas_off_leakage_power"][0])       

        print " after general ble block\n"
        sys.stdout.flush()          
        ...
        for lut_input_name, lut_input in self.logic_cluster.ble.lut.input_drivers.iteritems():
            ...
            driver.leak_power_state_on = float(spice_meas["meas_on_leakage_power"][0])
            driver.leak_power_state_off = float(spice_meas["meas_off_leakage_power"][0])                                     
            ...
            not_driver.leak_power_state_on = float(spice_meas["meas_on_leakage_power"][0])
            not_driver.leak_power_state_off = float(spice_meas["meas_off_leakage_power"][0])              
    ...
    def _generate_basic_subcircuits(self):
		...
		# Generate lvl_shifter_single_stage subcircuit
        basic_subcircuits.lvl_shifter_generate(self.basic_subcircuits_filename, self.specs.use_finfet)
    ...
    def _generate_process_data(self):
        ...
        if voltage_islands > 0:
            process_data_file.write(".PARAM supply_v_log = " + str(self.specs.vdd) + "\n")		
        ...
        if voltage_islands > 0:
            process_data_file.write("VSUPPLYLOGIC vdd_log gnd supply_v_log\n")		
    ...
    def _generate_process_data_ibrahim(self, file_name, default_vdd, logic_vdd):
        """ Write the process data library file. It contains voltage levels, gate length and device models. """
        
        print "Generating process data file"

        
        process_data_file = open(file_name, 'w')
        process_data_file.write("*** PROCESS DATA AND VOLTAGE LEVELS\n\n")
        process_data_file.write(".LIB PROCESS_DATA\n\n")
        process_data_file.write("* Voltage levels\n")
        process_data_file.write(".PARAM supply_v = " + str(default_vdd) + "\n")
        if voltage_islands > 0:
            process_data_file.write(".PARAM supply_v_log = " + str(logic_vdd) + "\n")		
